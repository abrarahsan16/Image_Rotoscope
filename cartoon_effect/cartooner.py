...
class CartoonEffect():
    def stylization(self, img, imgGrey, sigmaBlur, sigmaDog, p, epsilon, N):
        ...
        if img.ndim == 3:
            ...
            XDog = self.XDoG(img, sigmaDog, p, epsilon)
            #XDog[:, :, 1] = XDog[:, :, 0]
            #XDog[:, :, 2] = XDog[:, :, 0]
            return C * XDog
        else:
            C = C/N
            # Return output
            return C * self.XDoG(img, sigmaDog, p, epsilon)
    ...
    def XDoG(self, img, sigma, p, epsilon):
        # If image is RGB
        if img.ndim == 3:
        ...
        # Else, copy
            ...
            imgGrey = np.copy(img)
        # Apply gaussian blur on the image
        imgGaus = self.gaussianBlur(imgGrey, sigma)
        DoG = imgGaus - self.gaussianBlur(imgGrey, 1.6*sigma)
        U = imgGrey + p*DoG
        ...
        return self.LineCleanup(IXDoG.astype(np.float64))
        raise NotImplementedError('Implement this function/method.')
    ...
    def LineCleanup(self, img):
        ...
        Ehorz = self.convolve(img, horzKern)
        Evert = self.convolve(img, vertKern)
        C = np.logical_and(Ehorz>0, Evert>0)
        meanKern = (1/9)*np.full((3, 3), 1)
        B = self.convolve(img, meanKern)
        Ifilt = np.copy(img)
        Ifilt[C] = B[C]
        return Ifilt
        raise NotImplementedError('Implement this function/method.')
